...
struct CfgSvcMock::impl
{
  ...
  connector::server_context::sptr            mon_srv_contex_;
  ...
  : cfg_srv_contex_{new connector::server_context},
    ...
    mon_srv_contex_{new connector::server_context},
    ...
    sink_stderr_{new logger::log_sink{dummy_socket_}}
  {
    ...
    mon_srv_contex_->service_name("monitoring-service");
    mon_srv_contex_->endpoint_svc_addr(ep);
    mon_srv_contex_->ip_discovery_timeout_ms(1);
    ...
    timer_.schedule(util::DEFAULT_ENDPOINT_EXPIRY_MS/3, [this]() {
      ...
      mon_srv_contex_->keep_alive(*ep_clnt_);
      return true;
    });
  }
};
