  ...
  query_proxy::handle_query(connector::query_server::query_sptr q)
  {
    ...
    on_resend_chunk   resend_chunk_copy;
    {
      ...
      if( q->has_querycontrol() )
      {
        ...
        {
          server_ctx_->increase_stat("Query has STOP command (proxy)");
          stop_query = true;
        }
        else if( q->querycontrol() == interface::pb::Query::RESEND_CHUNK )
        {
          server_ctx_->increase_stat("Query has RESEND_CHUNK command (proxy)");
          resend_chunk = true;
          resend_chunk_copy = on_resend_chunk_;
        }
        else if( q->querycontrol() == interface::pb::Query::RESEND_TABLE )
        {
          server_ctx_->increase_stat("Query has RESEND_TABLE command (proxy)");
        }
