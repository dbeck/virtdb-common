...
namespace virtdb { namespace dsproxy {
  ...
  void
  query_proxy::watch_new_segments(on_new_segment m)
  {
    std::unique_lock<std::mutex> l(mtx_);
    on_new_segment_ = m;
  }
  ...
  query_proxy::string_set
  query_proxy::query_segments(const std::string & query_id) const
  {
    std::unique_lock<std::mutex> l(mtx_);
  ...
  void
  query_proxy::remove_segments(const std::string & query_id,
  ...
  void
  query_proxy::remove_query(const std::string & query_id)
  {
    std::unique_lock<std::mutex> l(mtx_);
